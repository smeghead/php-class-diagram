...
final class PhpReflectionTest extends TestCase {
    ...
    public function testDump(): void {
        ...
        $this->assertSame('Name', $data->getProperties()[0]->type->getName(), 'property name type.');
        $this->assertSame([], $data->getProperties()[0]->type->getNamespace(), 'namespace.');
        ...
        $this->assertSame('Price', $data->getProperties()[1]->type->getName(), 'property price type.');
        $this->assertSame([], $data->getProperties()[1]->type->getNamespace(), 'namespace.');
    ...
    public function testDump_Price(): void {
        ...
        $this->assertSame('int', $data->getProperties()[0]->type->getName(), 'property price type.');
        $this->assertSame([], $data->getProperties()[0]->type->getNamespace(), 'namespace.');
    ...
    public function testDump_with_namespace(): void {
        ...
        $this->assertSame('Name', $data->getProperties()[0]->type->getName(), 'type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getProperties()[0]->type->getNamespace(), 'namespace.');
        $this->assertSame('price', $data->getProperties()[1]->name, 'name.');
        $this->assertSame('Price', $data->getProperties()[1]->type->getName(), 'type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getProperties()[1]->type->getNamespace(), 'namespace.');
    }
    ...
    public function testDump_with_phpdoc(): void {
        ...
        $this->assertSame('name', $data->getProperties()[0]->name, 'type.');
        $this->assertSame('Name', $data->getProperties()[0]->type->getName(), 'type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getProperties()[0]->type->getNamespace(), 'Name namespace.');
        $this->assertSame('price', $data->getProperties()[1]->name, 'name.');
        $this->assertSame('Price', $data->getProperties()[1]->type->getName(), 'type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getProperties()[1]->type->getNamespace(), 'Price namespace.');
        $this->assertSame('Tag[]', $data->getProperties()[2]->type->getName(), 'type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getProperties()[2]->type->getNamespace(), 'Tag[] namespace.');
        $this->assertSame('Ban[]', $data->getProperties()[3]->type->getName(), 'full package name, Ban type.');
        $this->assertSame(['ban', 'ban', 'ban'], $data->getProperties()[3]->type->getNamespace(), 'full package name, ban.ban.ban.Ban[] namespace.');
    }
    ...
    public function testDump_with_interface(): void {
        ...
        $this->assertSame('string', $data->getProperties()[0]->type->getName(), 'property type.');
    }
    public function testDump_with_methods(): void {
        ...
        $this->assertSame('Product', $data->getMethods()[0]->type->getName(), 'return type.');
        $this->assertSame([], $data->getMethods()[0]->type->getNamespace(), 'return type namespace.');
    ...
    public function testDump_with_methods2(): void {
        ...
        $this->assertSame('method1', $data->getMethods()[0]->name, 'namespace name.');
        $this->assertSame('Product', $data->getMethods()[0]->type->getName(), 'return type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getMethods()[0]->type->getNamespace(), 'return type namespace.');
        $this->assertSame('param1', $data->getMethods()[0]->params[0]->name, 'parameter name.');
        $this->assertSame(true, $data->getMethods()[0]->accessModifier->public, 'public.');
        $this->assertSame(false, $data->getMethods()[0]->accessModifier->private, 'private.');
        $this->assertSame(false, $data->getMethods()[0]->accessModifier->static, 'static.');
    ...
    public function testNullable(): void {
        ...
        $this->assertSame('Tag', $data->getMethods()[0]->type->getName(), 'return type.');
        $this->assertSame(['hoge', 'fuga', 'product', 'tags'], $data->getMethods()[0]->type->getNamespace(), 'return type namespace.');
        $this->assertSame('name', $data->getMethods()[0]->params[0]->name, 'parameter name.');
        $this->assertSame('Name', $data->getMethods()[0]->params[0]->type->getName(), 'parameter type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getMethods()[0]->params[0]->type->getNamespace(), 'parameter type namespace.');
        $this->assertSame('tag', $data->getMethods()[0]->params[1]->name, 'parameter name. tag');
        $this->assertSame('Tag', $data->getMethods()[0]->params[1]->type->getName(), 'parameter type. tag');
        $this->assertSame(['hoge', 'fuga', 'product', 'tags'], $data->getMethods()[0]->params[1]->type->getNamespace(), 'parameter type namespace. tag');
    }
    public function testNullableWithoutNamespace(): void {
        ...
        $this->assertSame([], $data->getClassType()->getNamespace(), 'namespace name.');
        $this->assertSame('nullable', $data->getMethods()[0]->name, 'namespace name.');
        $this->assertSame('Name', $data->getMethods()[0]->type->getName(), 'return type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getMethods()[0]->type->getNamespace(), 'return type namespace.');
        $this->assertSame('name', $data->getMethods()[0]->params[0]->name, 'parameter name.');
        $this->assertSame('Name', $data->getMethods()[0]->params[0]->type->getName(), 'parameter type.');
        $this->assertSame(['hoge', 'fuga', 'product'], $data->getMethods()[0]->params[0]->type->getNamespace(), 'parameter type namespace.');
    ...
    public function testFullyQualified(): void {
        ...
        $this->assertSame('Exception', $data->getMethods()[0]->type->getName(), 'return type.');
        $this->assertSame([], $data->getMethods()[0]->type->getNamespace(), 'return type namespace.');
        $this->assertSame('e', $data->getMethods()[0]->params[0]->name, 'parameter name.');
        $this->assertSame('Exception', $data->getMethods()[0]->params[0]->type->getName(), 'parameter type.');
        $this->assertSame([], $data->getMethods()[0]->params[0]->type->getNamespace(), 'parameter type namespace.');
        $this->assertSame('external', $data->getMethods()[1]->name, 'method name.');
        $this->assertSame('Exception', $data->getMethods()[1]->type->getName(), 'return type.');
        $this->assertSame(['external'], $data->getMethods()[1]->type->getNamespace(), 'return type namespace.');
    }
